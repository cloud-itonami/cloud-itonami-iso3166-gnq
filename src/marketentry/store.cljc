(ns marketentry.store
  "SSoT for the Equatorial Guinea (GNQ) market-entry compliance actor,
...
(defn demo-data
  ...
  `:paid-in-capital-xaf` is ground truth for the flagship PAID-IN
  CAPITAL FLOOR VALIDATION check (World Bank Doing Business 2020 --
  Commercial Registry (Registro de la Propiedad y Mercantil) XAF
  1,000,000 SARL floor); `:requires-tax-id?` / `:tax-id-verified?` are
  ground truth for the conditional Tax Authorities (Public Treasury)
  tax-identification-number check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Malabo Comercio y Servicios SARL" :registration-channel "Registro de la Propiedad y Mercantil (Malabo)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :paid-in-capital-xaf 1000000
             :requires-tax-id? true :tax-id-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GNQ" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :registration-channel "Registro de la Propiedad y Mercantil (Malabo)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :paid-in-capital-xaf 1000000
             :requires-tax-id? true :tax-id-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Bata Import-Export SARL" :registration-channel "Registro de la Propiedad y Mercantil (Bata)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "GNQ" :status :intake}
    "eng-4" {:id "eng-4" :operator "Rio Muni Construcciones SARL" :registration-channel "Registro de la Propiedad y Mercantil (Malabo)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :paid-in-capital-xaf 500000
             :requires-tax-id? true :tax-id-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GNQ" :status :intake}
    "eng-5" {:id "eng-5" :operator "Annobón Logística SARL" :registration-channel "Registro de la Propiedad y Mercantil (Malabo)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :paid-in-capital-xaf 1000000
             :requires-tax-id? true :tax-id-verified? false
             :drafted? false :submitted? false
             :jurisdiction "GNQ" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator registration-channel base-fee monthly-rate monitoring-months claimed-fee
                               paid-in-capital-xaf
                               requires-tax-id? tax-id-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    paid-in-capital-xaf                            (assoc :engagement/paid-in-capital-xaf paid-in-capital-xaf)
    (some? requires-tax-id?)                       (assoc :engagement/requires-tax-id? requires-tax-id?)
    (some? tax-id-verified?)                       (assoc :engagement/tax-id-verified? tax-id-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/registration-channel :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/paid-in-capital-xaf
   :engagement/requires-tax-id? :engagement/tax-id-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :registration-channel (:engagement/registration-channel m)
     ...
     :paid-in-capital-xaf (:engagement/paid-in-capital-xaf m)
     :requires-tax-id? (boolean (:engagement/requires-tax-id? m))
     :tax-id-verified? (boolean (:engagement/tax-id-verified? m))
