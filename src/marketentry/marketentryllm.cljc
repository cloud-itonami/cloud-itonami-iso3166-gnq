(ns marketentry.marketentryllm
  ...
  the Equatorial Guinea (GNQ) company/business-registration
  ...
  cited), never a committed record or a real Commercial Registry
  (Registro de la Propiedad y Mercantil) / Ministry of Commerce filing.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world Commercial Registry (Registro
  de la Propiedad y Mercantil) / Ministry of Commerce filing
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "tax-id-verified?=" (:tax-id-verified? e)
                        " paid-in-capital-xaf=" (:paid-in-capital-xaf e)
