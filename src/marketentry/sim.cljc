...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (GNQ, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up capital-below-floor) ==")
    ...
    (println "== filing/submit eng-4 (declared paid-in capital below Commercial Registry's XAF 1,000,000 floor -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tax-id-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tax identification number unverified -> HARD hold) ==")
