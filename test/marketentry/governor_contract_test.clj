  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Malabo Comercio y Servicios SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Malabo Comercio y Servicios SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest capital-below-floor-is-held-and-unoverridable
  (testing "declared paid-in capital (XAF 500,000) below the Commercial Registry's World-Bank-documented XAF 1,000,000 floor -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:capital-below-floor} (-> (store/ledger db) last :basis)))
...
(deftest tax-id-unverified-is-held-and-unoverridable
  (testing "unverified Tax Authorities (Public Treasury) tax identification number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tax-id-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest capital-exactly-at-floor-is-not-held-by-the-flagship-check
  (testing "eng-1 declares paid-in capital exactly at the XAF 1,000,000 floor -- meeting the floor is fine, not a violation"
    (let [[_db actor] (fresh)
